...
!$omp end do
!$omp single
!	lai(:,:) = lai(:,:) / real(Max_loc) / real(Max_loc)          !!!>>>>>>>>>>>>TN:rm
!	lai_cum(:,:) = lai_cum(:,:) / real(Max_loc) / real(Max_loc)  !!!>>>>>>>>>>>>TN:rm
	lai(:,:) = lai(:,:) /  real(GRID%Area)          !!!<<<<<<<<<<<<TN:add
	lai_cum(:,:) = lai_cum(:,:) /  real(GRID%Area)  !!!<<<<<<<<<<<<TN:add
...
!_____________ Main(Method1)
	count = 0
!$omp parallel
!$omp do private(i,j,no,flag_coverage,x,y,d1,d2,d3,d4,d5,d6,d7,d8,d9,proxy),reduction(+:count)
...
!●_____________ ir_tree: Radiation-interception-coefficient by crown layer, lower=more absorb.
	a3 = 0.0
!$omp parallel
!$omp do private(no,p,a1,frac_absorb,i,a2),reduction(+:a3)
	DO no=1, Max_no
		p = pft(no)
		if ( .not. tree_exist(no)      ) cycle
...
!	a4 = (a3 / real(Max_loc) / real(Max_loc) ) / par   !!!>>>>>>>>>>>>TN:rm
	a4 = (a3 / real(GRID%Area) ) / par         !!!<<<<<<<<<<<<TN:add
...
!	cell_length = real(Max_loc   ) / real(DivedG   ) !Side length for each grass cell (m) !!!>>>>>>>>>>>>TN:rm
	cell_area   = GRID%CellArea   !Area for each grass cell (m2) !!!<<<<<<<<<<<<TN:add
	cell_length = GRID%CellLength !Side length for each grass cell (m) !!!<<<<<<<<<<<<TN:add
	
!$omp parallel
!$omp do private(loop_no,no,count,flag,i,x,d1,d2,d3,dist_x,j,y,dist_y,proxy)
...
!	a1 = sum(par_grass_rel(:,:)) / real(DivedG) / real(DivedG) !!!>>>>>>>>>>>>TN:rm
	a1 = sum(par_grass_rel(:,:)) / real(GRID%N_tot) !!!<<<<<<<<<<<<TN:add
...
!	a1 = (real(Max_loc)/real(DivedG)) ** 2.0 !!!>>>>>>>>>>>>TN:rm
	a1 = GRID%CellArea !!!<<<<<<<<<<<<TN:add
	
	a3 = 0.0
!$omp end single
!$omp do private(i,j,frac_absorb,a2),reduction(+:a3)
!	do i=1, DivedG !!!>>>>>>>>>>>>TN:rm
!	do j=1, DivedG !!!>>>>>>>>>>>>TN:rm
	do i=1, GRID%N_x !!!<<<<<<<<<<<<TN:add
	do j=1, GRID%N_y !!!<<<<<<<<<<<<TN:add
...
!	a4 = ( a3 / real(Max_loc*Max_loc) ) / (ir_tree*par) !!!>>>>>>>>>>>>TN:rm
	a4 = ( a3 / real(GRID%Area) ) / (ir_tree*par) !!!<<<<<<<<<<<<TN:add
...
!	integer,dimension(Max_no*Max_no)::save_value3 !!!>>>>>>>>>>>>TN:rm
!	integer,dimension(Max_no*Max_no)::save_value4 !!!>>>>>>>>>>>>TN:rm
	integer,allocatable,dimension(:)::save_value3 !!!<<<<<<<<<<<<TN:add
	integer,allocatable,dimension(:)::save_value4 !!!<<<<<<<<<<<<TN:add
	...
	allocate( save_value3(Max_no*Max_no) ) !!!<<<<<<<<<<<<TN:add for error handling
	allocate( save_value4(Max_no*Max_no) ) !!!<<<<<<<<<<<<TN:add for error handling
...
!$omp parallel
!$omp do private(no,i,j,x,y,d1,d2,d3,d4,d5,d6,d7,d8,d9,distance)
Do no=1, Max_no
	if ( .not. tree_exist(no)   ) cycle
	
	!two trees cannot exist at the same establishment location
!	x = real(Dived)/real(Max_loc) !!!>>>>>>>>>>>>TN:rm
	x = real(GRID%N_x)/real(GRID%Max_x) !!!<<<<<<<<<<<<TN:add
...
!		y = real(j) * real(Max_loc) / real(Dived) - 0.5 !y location (m) !!!>>>>>>>>>>>>TN:rm
		x = real(i) * real(GRID%Max_x) / real(GRID%N_x) - 0.5 !x location (m) !!!<<<<<<<<<<<<TN:add
		y = real(j) * real(GRID%Max_y) / real(GRID%N_y) - 0.5 !y location (m) !!!<<<<<<<<<<<<TN:add
...
!$omp parallel
!$omp do private(me,dx,dy,d,r,angle0,angle1,angle2,z,count1,you,a1,a2,a3), &
!$omp private(i,count2,distance,x,y,direction,angle)
DO me=1, Max_no
